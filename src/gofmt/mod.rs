...
fn decode_rune(s: &str, i: usize) -> char {
    s[i..].chars().next().unwrap_or(RUNE_ERROR)
}
...
///
/// Named rather than written as a literal so the value is taken from the
/// standard library's own definition of the replacement character instead of
/// being spelled out at each use site.
const RUNE_ERROR: char = char::REPLACEMENT_CHARACTER;
...
fn rune_or_error(c: u64) -> char {
    ...
    char::from_u32(c as u32).unwrap_or(RUNE_ERROR)
}
